    ...
    <div id="submit">
        <?php if (isset($_SESSION['timer']) && time() - $_SESSION['timer'] < $timejg) { ?>
            <div id="alertmsg">
                ...
                秒后再留言……您还需等待：<?php echo abs($timejg - (time() - $_SESSION['timer'])) ?>秒<br>
            ...
            if (empty($_POST['ac'])) {
                ...
                <form name="form1" method="post" action="<?php $_SERVER['PHP_SELF'] ?>"
                    ...
                    <label for="user">昵称：</label><input type="text" id=username name="username" value="<?php echo $_SESSION['nickname']?>" disabled/>*<br/>
                    <label for="comment">内容：</label><textarea id=content name="content" style="resize: none"></textarea>*<br/>
                    <label for="email">悄悄话：</label>
                    <input name="ifqqh" type="checkbox" id="ifqqh" value="1"> <span>当选中时，此留言只有管理员可见</span><br/>
                    <label for="umum">验证码：</label><input name="unum" type="text" id="unum" size="10">* <img
                            src="include/randnum.php?id=-1" title="点击刷新" style="cursor:pointer"
                            onclick=eval('this.src="include/randnum.php"')><br/>
                    <input type="submit" id="sbutton" value="确  定"/><br/><input name="ac" type="hidden" id="ac"
                                                                                value="add">
                </form>
                ...
                <div id="alertmsg">
                    ...
                    if ($_POST['unum'] == $_SESSION["randValid"]) {
                        ...
                        if (!empty($content) or !empty($username)) {
                            ...
                            $sql = "insert into " . TABLE_PREFIX . "message(username,content,systime,ifshow,ifqqh) values('" . $username . "','" . $content . "','" . $systime . "'," . $ifshow . "," . $ifqqh . ")";
                            if (($db->execute($sql)) > 0) {
                                $_SESSION['timer'] = time();
